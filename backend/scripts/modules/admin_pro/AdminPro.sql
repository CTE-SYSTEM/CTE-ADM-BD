...
RETURNS BOOLEAN AS $$
BEGIN
    -- Retorna TRUE si han pasado menos de 30 minutos
...
BEGIN
    -- Obtenemos la fecha en la que se grabó la asignación
    ...
    -- Verificamos si pasaron más de 30 min desde la asignación
    IF v_fecha_asignacion >= (NOW() - INTERVAL '30 minutes') THEN
        UPDATE "Diagnosticos"
        SET tecnico_id = p_nuevo_tecnico_id,
            fecha_asignacion = NOW() -- Opcional: resetear el reloj al editar
        WHERE id_diagnostico = p_id_diagnostico;
...
CREATE OR REPLACE FUNCTION admin_pro.editar_aprobacion_repuesto(
    ...
    p_nuevo_estado TEXT -- 'APROBADO', 'DENEGADO', 'PENDIENTE'
)
RETURNS JSON AS $$
DECLARE
    v_fecha_orden TIMESTAMP;
BEGIN
    -- Buscamos la fecha de la orden para saber si es reciente
